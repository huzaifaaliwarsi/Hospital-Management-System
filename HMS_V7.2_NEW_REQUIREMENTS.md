...
> Continue the HMS v7.2 work per `HMS_V7.2_NEW_REQUIREMENTS.md`. Super Admin's v7.2 slice (§3.1) is fully done. Front Desk is 12/13 nav items real (only `panel_billing` deferred). **§2.2 Department Sub-Invoice Split is now DONE** (session 7) — the open design question is resolved and implemented: no new model, `HospitalInvoice` reused per-(admission × department), one row per department, `admission.service.ts`'s `addAdmissionService` routes each posted service to its own service's department invoice, and a new `frontdesk/admissionBilling.*` module gives Front Desk the Interim Statement (§2.10) + Payment Allocation (§2.11) over all of an admission's department invoices. **Live-verified end-to-end against the real dev DB** (not just mocked tests): a real 2-department admission (Administration + General Medicine) posted correctly to 2 separate `HospitalInvoice` rows, a PKR 2,000 payment auto-allocated proportionally to 909.09/1090.91 (sums exactly, no rounding drift), both invoices flipped to `PARTIALLY_PAID`, and the cashier's balance sheet showed the correct un-split PKR 2,000 physical total. 60/60 backend tests passing (8 new this session), `tsc --noEmit` + `npm run build` clean both sides. **Panel Billing (Panel Verification, Contract Resolution, Interim Statement, Remittance) is now unblocked and is the natural next piece** — confirmed directly against the source PDFs (`13 Department Billing Split and Settlement.pdf`, `16 Panel Patient Complete Flow.pdf`) that it structurally depends on §2.2, which now exists; still needs its own build pass (Panel Remittance has no model yet — the plan is to allocate remittance at the existing per-department `HospitalInvoice` level, per session 7's resolved design, not wait for anything further). **Standing safety rule from session 6 (still in force):** never point `prisma migrate diff`'s `--shadow-database-url` at a database with real data — use `--from-schema-datamodel <old-schema-copy>`/`--to-schema-datamodel <new-schema>` (pure static file diff, zero DB connection) instead, exactly as sessions 6 and 7 both did. Ask whether to build Panel Billing next, or default to whichever the user names.
