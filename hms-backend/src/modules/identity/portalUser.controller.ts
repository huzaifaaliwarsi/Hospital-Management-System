...
export const portalUserController = {
  ...
  async create(req: Request, res: Response) {
    const user = await portalUserService.create(req.body as CreatePortalUserBody, actorId(req));
    res.status(201).json({ data: user });
  },

  async update(req: Request, res: Response) {
    const user = await portalUserService.update(req.params.id as string, req.body as UpdatePortalUserBody, actorId(req));
    res.json({ data: user });
  },
  ...
  async resetPassword(req: Request, res: Response) {
    ...
    const user = await portalUserService.resetPassword(req.params.id as string, newPassword, actorId(req));
    res.json({ data: user });
  },

  async remove(req: Request, res: Response) {
    await portalUserService.remove(req.params.id as string);
    res.status(204).send();
  },
};
